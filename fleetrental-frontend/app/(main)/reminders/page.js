...
import { Bell, Plus, Edit2, Trash2, Search, Car, AlertCircle, XCircle, ChevronDown, AlertTriangle, Clock, CheckCircle2, RotateCcw } from 'lucide-react';

const API_URL = process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000/api';

const REMINDER_TYPES = [
    'Vidange', 'Freins', 'Pneus', 'Batterie', 'Révision',
    'Climatisation', 'Transmission', 'Embrayage', 'Échappement',
    'Contrôle technique', 'Assurance', 'Autre',
];
...
const STATUS_CONFIG = {
    overdue:  { label: 'En retard',  textColor: 'text-red-700',   bgColor: 'bg-red-50',   icon: AlertTriangle },
    upcoming: { label: 'À venir',    textColor: 'text-amber-700', bgColor: 'bg-amber-50', icon: Clock },
    ok:       { label: 'OK',         textColor: 'text-green-700', bgColor: 'bg-green-50', icon: CheckCircle2 },
};
...
export default function RemindersPage() {
    ...
    const [renewForm, setRenewForm] = useState({ mileage_interval: '', date_months: '' });
    ...
    const handleSave = async () => {
        ...
        try {
            const payload = {
                ...form,
                next_due_mileage: form.next_due_mileage === '' ? null : form.next_due_mileage,
    ...
    const handleDelete = async (id) => {
        ...
        await fetch(`${API_URL}/reminders/${id}`, { method: 'DELETE', headers: headers() });
        await refreshReminders();
    };
    ...
    const openRenewModal = (reminder) => {
        const defaults = DEFAULT_INTERVALS[reminder.type] || DEFAULT_INTERVALS['Autre'];
    ...
    const handleRenew = async () => {
        ...
        setError('');
        try {
            const res = await fetch(`${API_URL}/reminders/${renewingReminder.id}/renew`, {
                method: 'POST',
                headers: headers(),
    ...
    const filtered = reminders.filter(r => {
        const matchSearch = search === '' ||
            `${r.type} ${r.vehicle?.make || ''} ${r.vehicle?.model || ''} ${r.description || ''}`.toLowerCase().includes(search.toLowerCase());
        const matchVehicle = filterVehicle === 'all' || r.vehicle_id === parseInt(filterVehicle);
        const matchStatus = filterStatus === 'all' || r.computed_status === filterStatus;
        return matchSearch && matchVehicle && matchStatus;
    });

    const overdueCount  = reminders.filter(r => r.computed_status === 'overdue').length;
    const upcomingCount = reminders.filter(r => r.computed_status === 'upcoming').length;
    ...
    return (
        <div>
            ...
            <div className="flex items-center justify-between mb-8">
                <div>
                    <h1 className="text-2xl font-bold text-gray-800">Rappels de maintenance</h1>
                    <p className="text-gray-400 text-sm mt-1">{reminders.length} rappel(s) actif(s)</p>
                </div>
                {isAdmin && (
                    <button
            ...
            <div className="flex items-center gap-3 mb-5">
                ...
                {(filterVehicle !== 'all' || search || filterStatus !== 'all') && (
                    <button
                        onClick={() => { setFilterVehicle('all'); setSearch(''); setFilterStatus('all'); }}
                    ...
                    <table className="w-full">
                        <thead>
                            <tr className="border-b border-gray-100 bg-gray-50">
                                <th className="text-left px-6 py-3.5 text-xs font-bold text-gray-400 uppercase tracking-wider">Véhicule</th>
                                <th className="text-left px-6 py-3.5 text-xs font-bold text-gray-400 uppercase tracking-wider">Type</th>
                                <th className="text-left px-6 py-3.5 text-xs font-bold text-gray-400 uppercase tracking-wider">Statut</th>
                                <th className="text-left px-6 py-3.5 text-xs font-bold text-gray-400 uppercase tracking-wider">Kilométrage</th>
                                <th className="text-left px-6 py-3.5 text-xs font-bold text-gray-400 uppercase tracking-wider">Date limite</th>
                                <th className="text-left px-6 py-3.5 text-xs font-bold text-gray-400 uppercase tracking-wider">Actions</th>
                            </tr>
                        </thead>
                        <tbody>
                            {filtered.map((r) => {
                                const status = STATUS_CONFIG[r.computed_status];
                                const StatusIcon = status.icon;
                                const vehicle = r.vehicle;
                                const mileageLeft = r.next_due_mileage ? r.next_due_mileage - vehicle.mileage : null;
                                const canRenew = r.computed_status === 'overdue' || r.computed_status === 'upcoming';
                                    ...
                                    <tr key={r.id} className="border-b border-gray-50 hover:bg-gray-50 transition">
                                        <td className="px-6 py-4">
                                            <div className="flex items-center gap-3">
                                                ...
                                                <div>
                                                    <div className="text-sm font-semibold text-gray-800">{vehicle?.make} {vehicle?.model}</div>
                                                    <div className="text-xs text-gray-400">{vehicle?.license_plate}</div>
                                                </div>
                                            </div>
                                        </td>
                                        <td className="px-6 py-4">
                                        ...
                                        <td className="px-6 py-4">
                                            <span className={`inline-flex items-center gap-1.5 text-xs font-semibold px-2.5 py-1.5 rounded-lg ${status.bgColor} ${status.textColor}`}>
                                                <StatusIcon size={12} />
                                                {status.label}
                                            </span>
                                        </td>
                                            ...
                                            {r.next_due_mileage ? (
                                                <div>
                                                    <div className="text-sm font-semibold text-gray-800">
                                                        {Number(r.next_due_mileage).toLocaleString()} <span className="text-gray-400 font-normal text-xs">km</span>
                                                    </div>
                                                    <div className={`text-xs mt-0.5 ${mileageLeft <= 0 ? 'text-red-500 font-semibold' : mileageLeft <= 500 ? 'text-amber-500 font-semibold' : 'text-gray-400'}`}>
                                                        {mileageLeft <= 0
                                        ...
                                        <td className="px-6 py-4">
                                            {r.next_due_date ? (
                                                <div>
                                                    <div className="text-sm font-semibold text-gray-800">
                                                        {new Date(r.next_due_date).toLocaleDateString('fr-FR', { day: '2-digit', month: 'short', year: 'numeric' })}
                                                    </div>
                                                    <div className={`text-xs mt-0.5 ${new Date(r.next_due_date) < new Date() ? 'text-red-500 font-semibold' : 'text-gray-400'}`}>
                ...
                <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50 px-4">
                    <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
                        <div className="flex items-center justify-between p-6 border-b border-gray-100">
                            <div className="flex items-center gap-3">
                                <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${editingId ? 'bg-blue-50' : 'bg-amber-50'}`}>
                                    {editingId
                                        ? <Edit2 size={18} className="text-blue-600" />
                                        : <Bell size={18} className="text-amber-600" />}
                                </div>
                                <div>
                                    <h2 className="font-bold text-gray-800">{editingId ? 'Modifier' : 'Nouveau'} rappel</h2>
                                    <p className="text-xs text-gray-400">Définissez par kilométrage, date, ou les deux</p>
                                </div>
                            </div>
                        ...
                        <div className="p-6 space-y-4">
                            ...
                            <div className="bg-blue-50 border border-blue-200 rounded-xl px-4 py-3 flex items-center gap-3">
                                ...
                                <p className="text-xs text-blue-700">
                                    🔴 <strong>En retard</strong> si dépassé · 🟡 <strong>À venir</strong> si &lt; 500 km ou 7 jours · 🟢 <strong>OK</strong> sinon
                                </p>
                            </div>

                            <div>
                                <label className="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-1.5">Véhicule</label>
                                <div className="relative">
                                    <Car size={16} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-gray-300" />
                            ...
                            <div>
                                ...
                                <div className="relative">
                                    <Bell size={16} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-gray-300" />
                            ...
                            <div>
                                ...
                                <textarea value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })}
                                    placeholder="Ex: Vidange à faire tous les 10 000 km..." rows={2}
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    <label className="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-1.5">
                                        Kilométrage limite <span className="ml-1 font-normal text-gray-300 normal-case">(optionnel)</span>
                                    </label>
                                    <input type="number" value={form.next_due_mileage} onChange={(e) => setForm({ ...form, next_due_mileage: e.target.value })}
                                ...
                                <div>
                                    <label className="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-1.5">
                                        Date limite <span className="ml-1 font-normal text-gray-300 normal-case">(optionnel)</span>
                                    </label>
                        ...
                        <div className="flex gap-3 p-6 border-t border-gray-100">
                            <button onClick={handleSave} disabled={saving}
                                ...
                                {saving ? 'Sauvegarde...' : editingId ? 'Mettre à jour' : 'Créer le rappel'}
                            </button>
                    ...
                    <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md">
                        <div className="flex items-center justify-between p-6 border-b border-gray-100">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <h2 className="font-bold text-gray-800">Renouveler le rappel</h2>
                                    <p className="text-xs text-gray-400">{renewingReminder.type} · {renewingReminder.vehicle?.make} {renewingReminder.vehicle?.model}</p>
                                </div>
                            </div>
                            <button onClick={() => setShowRenewModal(false)} className="text-gray-300 hover:text-gray-500 p-1 rounded-lg hover:bg-gray-100 transition">
                                <XCircle size={20} />
                            </button>
                        </div>
                        ...
                        <div className="p-6 space-y-4">
                            ...
                            <div className="bg-gray-50 border border-gray-200 rounded-xl p-4 space-y-2">
                                <div className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">Situation actuelle</div>
                                <div className="flex items-center justify-between">
                                    <span className="text-xs text-gray-500">Kilométrage véhicule</span>
                                    <span className="text-xs font-bold text-gray-800">{Number(renewingReminder.vehicle?.mileage).toLocaleString()} km</span>
                                </div>
                                {renewingReminder.next_due_mileage && (
                                    <div className="flex items-center justify-between">
                                        <span className="text-xs text-gray-500">Limite actuelle</span>
                                        <span className="text-xs font-bold text-red-600">{Number(renewingReminder.next_due_mileage).toLocaleString()} km</span>
                                    </div>
                                )}
                                {renewingReminder.next_due_date && (
                                    <div className="flex items-center justify-between">
                                        ...
                                        <span className="text-xs font-bold text-red-600">
                                            {new Date(renewingReminder.next_due_date).toLocaleDateString('fr-FR', { day: '2-digit', month: 'short', year: 'numeric' })}
                                        </span>
                                    </div>
                                )}
                            </div>
                                ...
                                <div>
                                    ...
                                    <div className="relative">
                                        <input
                                            ...
                                            onChange={(e) => setRenewForm({ ...renewForm, mileage_interval: e.target.value })}
                                        ...
                                        />
                                        <span className="absolute right-3.5 top-1/2 -translate-y-1/2 text-xs text-gray-400 font-semibold">km</span>
                                    </div>
                                    <p className="text-xs text-gray-400 mt-1.5">
                                        Nouvelle limite : <span className="font-bold text-blue-600">
                                            {renewForm.mileage_interval
                                                ? `${(parseInt(renewingReminder.vehicle?.mileage) + parseInt(renewForm.mileage_interval)).toLocaleString()} km`
                                                : '—'}
                                        </span>
                                    </p>
                                </div>
                                ...
                                <div>
                                    ...
                                    <div className="relative">
                                        <input
                                            ...
                                            onChange={(e) => setRenewForm({ ...renewForm, date_months: e.target.value })}
                                            className="w-full px-3 pr-16 py-2.5 rounded-xl border border-gray-200 focus:border-blue-500 focus:ring-4 focus:ring-blue-100 outline-none text-sm text-gray-800 bg-gray-50 focus:bg-white transition"
                                        />
                                        <span className="absolute right-3.5 top-1/2 -translate-y-1/2 text-xs text-gray-400 font-semibold">mois</span>
                                    </div>
                                    <p className="text-xs text-gray-400 mt-1.5">
                                        Nouvelle date : <span className="font-bold text-blue-600">
                                            {renewForm.date_months
                                                ? new Date(Date.now() + parseInt(renewForm.date_months) * 30 * 86400000).toLocaleDateString('fr-FR', { day: '2-digit', month: 'short', year: 'numeric' })
                                                : '—'}
                                        </span>
                                    </p>
                                </div>
