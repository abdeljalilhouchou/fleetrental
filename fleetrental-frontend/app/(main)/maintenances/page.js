...
import {
    Wrench, Plus, Edit2, Trash2, Search, Car, AlertCircle, XCircle,
    ChevronDown, CheckCircle2, Clock, Paperclip, Upload, FileText,
    FileImage, X, Eye, ChevronRight
} from 'lucide-react';
...
const MAINTENANCE_TYPES = [
    ...
    'Climatisation', 'Transmission', 'Embrayage', 'Échappement', 'Autre',
];
...
export default function MaintenancesPage() {
    ...
    const handleFileUpload = async (maintenanceId, files) => {
        ...
        try {
            const res = await fetch(`${API_URL}/maintenances/${maintenanceId}/files`, {
                method: 'POST',
    ...
    const handleDeleteFile = async (maintenanceId, fileId) => {
        ...
        const res = await fetch(`${API_URL}/maintenances/${maintenanceId}/files/${fileId}`, {
            method: 'DELETE',
            headers: headers(),
        });
        if (res.ok) await refreshMaintenances();
    ...
    const filtered = maintenances.filter(m => {
        ...
        const matchSearch = `${m.type} ${m.description} ${vehicleName}`.toLowerCase().includes(search.toLowerCase());
        const matchVehicle = filterVehicle === 'all' || m.vehicle_id === parseInt(filterVehicle);
    ...
    return (
        <div>
            <div className="flex items-center justify-between mb-8">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p className="text-gray-400 text-sm mt-0.5">Suivez l'entretien de vos véhicules</p>
                    </div>
                </div>
            ...
            <div className="grid grid-cols-3 gap-4 mb-6">
                ...
                <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
                    <div className="text-3xl font-bold text-blue-600">{totalCost.toLocaleString()} MAD</div>
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                {filtered.length === 0 ? (
                    <div className="text-center py-16">
                        ...
                        <p className="text-gray-500 font-semibold">Aucune maintenance trouvée</p>
                    </div>
                ) : (
                    <table className="w-full">
                        <thead>
                            <tr className="border-b border-gray-100 bg-gray-50">
                                <th className="text-left px-4 py-3.5 text-xs font-bold text-gray-400 uppercase">Type</th>
                                <th className="text-left px-4 py-3.5 text-xs font-bold text-gray-400 uppercase">Véhicule</th>
                                <th className="text-left px-4 py-3.5 text-xs font-bold text-gray-400 uppercase">Date</th>
                                <th className="text-left px-4 py-3.5 text-xs font-bold text-gray-400 uppercase">Coût</th>
                                <th className="text-left px-4 py-3.5 text-xs font-bold text-gray-400 uppercase">Statut</th>
                                <th className="text-left px-4 py-3.5 text-xs font-bold text-gray-400 uppercase">Actions</th>
                            </tr>
                        </thead>
                        <tbody>
                            {filtered.map((m) => {
                                            ...
                                            <tr>
                                                <td colSpan="6" className="bg-gray-50 px-4 py-4">
                                                    <div className="max-w-4xl">
                                                        ...
                                                        <div className="mb-4">
                                                            <div className="text-xs font-bold text-gray-400 uppercase mb-2">Fichiers joints</div>
                                                            {m.files && m.files.length > 0 ? (
                                                                <div className="grid grid-cols-2 gap-2 mb-3">
                                                                    {m.files.map(file => (
                                                                        <div key={file.id} className="flex items-center gap-2 bg-white rounded-lg border border-gray-200 p-2">
                                                                            <FileIcon mimeType={file.mime_type} />
                                                                            <div className="flex-1 min-w-0">
                                                                                <div className="text-xs font-medium text-gray-700 truncate">{file.file_name}</div>
                                                                                <div className="text-xs text-gray-400">{formatSize(file.file_size)}</div>
                                                                            </div>
                                                                            <a href={`${STORAGE_URL}${file.file_path}`} target="_blank" rel="noopener noreferrer"
                                                                                className="p-1 text-blue-600 hover:bg-blue-50 rounded">
                                                                                <Eye size={14} />
                                                                            </a>
                                                                            {isAdmin && (
                                                                                <button onClick={() => handleDeleteFile(m.id, file.id)}
                                                                ...
                                                                <div className="text-xs text-gray-400 mb-3">Aucun fichier joint</div>
                                                            )}
                                                            ...
                                                            <div
                                                                ...
                                                                onClick={() => {
                                                                    ...
                                                                    input.type = 'file';
                                                                    input.multiple = true;
                                                                    input.onchange = (e) => handleFileUpload(m.id, e.target.files);
                                                                    input.click();
                                                                }}>
                ...
                <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50 px-4">
                    <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg">
                        <div className="p-6 border-b border-gray-100">
                            <h2 className="font-bold text-gray-800">{editingId ? 'Modifier' : 'Nouvelle'} maintenance</h2>
                        </div>
                        ...
                        <div className="p-6 max-h-[60vh] overflow-y-auto">
                            ...
                            <div className="space-y-4">
                                <div>
                                    <label className="block text-xs font-bold text-gray-400 uppercase mb-2">Véhicule</label>
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <div>
                                        <label className="block text-xs font-bold text-gray-400 uppercase mb-2">Coût (MAD)</label>
                                ...
                                <div>
                                    ...
                                    <input type="number" value={form.mileage_at_maintenance} onChange={(e) => setForm({...form, mileage_at_maintenance: e.target.value})}
                                        className="w-full px-4 py-2.5 bg-gray-50 rounded-xl border border-gray-200 focus:border-blue-500 focus:ring-4 focus:ring-blue-100 outline-none text-sm" />
                                </div>
                            </div>
                        </div>
