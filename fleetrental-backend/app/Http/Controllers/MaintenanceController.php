...
class MaintenanceController extends Controller
{
    ...
    public function store(Request $request)
    {
        if (!$request->user()->hasPermission('create_maintenances')) {
            return response()->json(['message' => 'Permission refus�e : create_maintenances'], 403);
        }
    ...
    public function update(Request $request, Maintenance $maintenance)
    {
        if (!$request->user()->hasPermission('edit_maintenances')) {
            return response()->json(['message' => 'Permission refus�e : edit_maintenances'], 403);
        }
    ...
    public function complete(Maintenance $maintenance)
    {
        if (!request()->user()->hasPermission('complete_maintenances')) {
            return response()->json(['message' => 'Permission refus�e : complete_maintenances'], 403);
        }
    ...
    public function destroy(Maintenance $maintenance)
    {
        if (!request()->user()->hasPermission('delete_maintenances')) {
            return response()->json(['message' => 'Permission refus�e : delete_maintenances'], 403);
        }
