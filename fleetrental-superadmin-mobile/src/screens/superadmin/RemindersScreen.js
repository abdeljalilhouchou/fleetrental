...
function ReminderFormModal({ visible, reminder, vehicles, onClose, onSaved }) {
    ...
    React.useEffect(() => {
        if (visible) {
            if (reminder) {
                ...
                setVehicleId(''); setType('oil_change'); setDescription('');
                setNextDueDate(''); setNextDueMileage('');
            }
        }
    }, [visible, reminder]);
    ...
    const handleSave = async () => {
        ...
        setSaving(true);
        try {
            if (editing) { await updateReminder(reminder.id, payload); }
                    ...
                    <ScrollView style={fm.body} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
                        ...
                        <TouchableOpacity style={fm.picker} onPress={() => setShowVehiclePicker(true)}>
                            ...
                            <Text style={[fm.pickerText, !selectedVehicle && { color: '#94a3b8' }]}>
                                {selectedVehicle ? `${selectedVehicle.brand} ${selectedVehicle.model}` : 'Sélectionner un véhicule'}
                            </Text>
                        ...
                        <ScrollView>
                            {vehicles.map(v => (
                                ...
                                >
                                    ...
                                    <Text style={pk.itemSub}>{v.registration_number}</Text>
                                </TouchableOpacity>
                            ))}
                        </ScrollView>
                    </View>
        ...
        <Modal visible={visible} transparent animationType="slide" onRequestClose={onClose}>
            <KeyboardAvoidingView style={rn.overlay} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
                <View style={rn.sheet}>
                    ...
                    <Text style={rn.sub}>Définissez la prochaine échéance à partir des valeurs actuelles du véhicule.</Text>

                    <Text style={rn.label}>Intervalle kilométrique (km)</Text>
                    <TextInput style={rn.input} value={mileageInterval} onChangeText={setMileageInterval} placeholder="ex: 10000" placeholderTextColor="#cbd5e1" keyboardType="numeric" />

                    <Text style={rn.label}>Dans combien de mois</Text>
                    <TextInput style={rn.input} value={dateMonths} onChangeText={setDateMonths} placeholder="ex: 6" placeholderTextColor="#cbd5e1" keyboardType="numeric" />
...
export default function RemindersScreen() {
        ...
        Alert.alert(
            ...
            [
                { text: 'Annuler', style: 'cancel' },
                {
                    ...
                    onPress: async () => {
                        try { await deleteReminder(r.id); load(true); }
    ...
    const renderItem = ({ item: r }) => {
        ...
        return (
            <View style={[styles.card, { borderLeftColor: s.color, borderLeftWidth: 4 }]}>
                <View style={styles.cardTop}>
                    ...
                    <View style={{ flex: 1 }}>
                        ...
                        <Text style={styles.vehicleLabel}>
                            {v ? `${v.brand} ${v.model}` : '—'}{v?.registration_number ? ` · ${v.registration_number}` : ''}
                        </Text>
                    </View>
                ...
                {r.description ? <Text style={styles.description} numberOfLines={2}>{r.description}</Text> : null}
                ...
                <View style={styles.cardActions}>
                    <TouchableOpacity style={styles.actionBtn} onPress={() => { setRenewTarget(r); setRenewVisible(true); }}>
                        <Ionicons name="refresh-outline" size={14} color="#16a34a" />
                        <Text style={[styles.actionBtnText, { color: '#16a34a' }]}>Renouveler</Text>
                    </TouchableOpacity>
                    <TouchableOpacity style={styles.actionBtn} onPress={() => { setEditReminder(r); setFormVisible(true); }}>
    ...
    return (
        <SafeAreaView style={styles.safe}>
            ...
            <TouchableOpacity style={styles.fab} onPress={() => { setEditReminder(null); setFormVisible(true); }} activeOpacity={0.85}>
...
const fm = StyleSheet.create({
    ...
    sectionTitle: { fontSize: 14, fontWeight: '800', color: PRIMARY, marginTop: 20, marginBottom: 4, paddingTop: 12, borderTopWidth: 1, borderTopColor: '#f1f5f9' },
    picker: { flexDirection: 'row', alignItems: 'center', gap: 10, borderWidth: 1.5, borderColor: '#e2e8f0', borderRadius: 12, paddingHorizontal: 14, paddingVertical: 13, backgroundColor: '#f8fafc' },
    pickerText: { flex: 1, fontSize: 14, color: '#0f172a' },
    input: { borderWidth: 1.5, borderColor: '#e2e8f0', borderRadius: 12, paddingHorizontal: 14, paddingVertical: 12, fontSize: 14, color: '#0f172a', backgroundColor: '#f8fafc' },
    textArea: { height: 80, textAlignVertical: 'top' },
    footer: { flexDirection: 'row', gap: 10, padding: 20, borderTopWidth: 1, borderTopColor: '#f1f5f9' },
    cancelBtn: { flex: 1, paddingVertical: 14, borderRadius: 12, borderWidth: 1.5, borderColor: '#e2e8f0', alignItems: 'center' },
    cancelText: { fontSize: 14, fontWeight: '700', color: '#64748b' },
    saveBtn:    { flex: 2, paddingVertical: 14, borderRadius: 12, backgroundColor: PRIMARY, alignItems: 'center' },
...
const rn = StyleSheet.create({
    ...
    cancelBtn: { flex: 1, paddingVertical: 14, borderRadius: 12, borderWidth: 1.5, borderColor: '#e2e8f0', alignItems: 'center' },
    cancelText: { fontSize: 14, fontWeight: '700', color: '#64748b' },
    renewBtn: { flex: 2, paddingVertical: 14, borderRadius: 12, backgroundColor: '#16a34a', alignItems: 'center', flexDirection: 'row', justifyContent: 'center', gap: 8 },
