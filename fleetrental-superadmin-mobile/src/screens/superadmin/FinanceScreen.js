    ...
    return (
        <SafeAreaView style={styles.safe}>
            <ScrollView
                ...
                refreshControl={<RefreshControl refreshing={refreshing} onRefresh={() => load(period, true)} colors={[ACCENT]} />}
            >
                ...
                <View style={styles.card}>
                    {(data?.monthly_data || []).slice(-6).map((m, i) => (
                        <View key={i} style={styles.monthRow}>
                            ...
                            <Text style={[styles.monthProfit, { color: m.profit >= 0 ? '#16a34a' : '#dc2626' }]}>
                                {m.profit >= 0 ? '+' : ''}{Number(m.profit).toFixed(0)}
                            </Text>
                        </View>
                    ))}
...
const styles = StyleSheet.create({
    ...
    monthProfit:{ fontSize: 11, fontWeight: '800', width: 46, textAlign: 'right' },
